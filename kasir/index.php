...
?>
<section class="content-with-sidebar" data-kasir-refresh>
    ...
    <div class="page-title"><h1>Antrian Kasir</h1><span class="badge badge-black">Auto refresh 20s</span></div>
    <h2>Siap Dibayar</h2>
    <div class="grid grid-3 order-queue">
        <?php foreach ($ready->fetchAll() as $o): ?><article class="card order-ticket"><div class="code"><?= e($o['order_code']) ?></div><p><?= e($o['customer_name']) ?> - <?= e($o['order_type']) ?></p><p><?= (int) $o['items_count'] ?> item - <?= time_ago($o['created_at']) ?></p><h3><?= format_rupiah((float) $o['total']) ?></h3><a class="btn btn-primary" href="process.php?order_id=<?= (int) $o['id'] ?>">Proses Pembayaran</a></article><?php endforeach; ?>
    </div>
    <h2>Sedang Diproses</h2>
    <div class="grid grid-3 order-queue">
        <?php foreach ($active->fetchAll() as $o): ?><article class="card order-ticket"><div class="code"><?= e($o['order_code']) ?></div><p><?= e($o['customer_name']) ?> - <?= get_status_label($o['status']) ?></p><p><?= (int) $o['items_count'] ?> item - <?= time_ago($o['created_at']) ?></p><h3><?= format_rupiah((float) $o['total']) ?></h3></article><?php endforeach; ?>
    </div>
</section>
